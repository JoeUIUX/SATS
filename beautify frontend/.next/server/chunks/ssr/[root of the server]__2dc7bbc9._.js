...
async function mccifSet(sock, parameter, value) {
    ...
    if (sock && typeof sock.simulateRead === 'function') {
        mccLogger.debug(`Using simulation mode to set ${parameter}=${value}`);
    ...
    // If no valid socket is available
    const error = new Error("No valid socket connection available");
    mccLogger.error(error.message);
    throw error;
}
async function mccifRead(sock, parameters) {
    // Check if we're in simulation mode (old API)
    if (sock && typeof sock.simulateRead === 'function') {
        mccLogger.debug(`Using simulation API to read ${parameters.length} parameters`);
    ...
    if (sock && typeof sock.send === 'function' && typeof sock.receive === 'function') {
        try {
            ...
            mccLogger.error(`MCC read error: ${error}`);
            throw error;
        }
    }
    // If we're in development mode without a real server or proper simulation, return hardcoded values
    mccLogger.warn("No valid socket connection available, using fallback simulated values");
...
const MainScreen = ({ openToTestList, closeToTestList, openServerWindow, openModelWindow, closeModelWindow, showToTestList, showThreeDModelWindow, threeDModelProfileId, windowZIndexes, bringWindowToFront, zIndexCounter// ✅ Use counter from props
    ...
    // Add this function to handle the Start Test button click
    const handleStartTest = ()=>{
        console.log("🚀 Starting checkout test with items:", droppedItems);
        setShowCheckoutTest(true);
        // Bring any other windows to the front if needed
        if (bringWindowToFront) {
            setTimeout(()=>bringWindowToFront("CheckoutTest"), 100);
        }
    };
    // Add this function to handle closing the CheckoutTestProgress window
    ...
    // Add this useEffect to initialize the MCC socket when needed
    (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$server$2f$route$2d$modules$2f$app$2d$page$2f$vendored$2f$ssr$2f$react$2e$js__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["useEffect"])(()=>{
        if (showCheckoutTest && !mccSocket) {
            ...
            const initMccSocket = async ()=>{
                try {
                    ...
                    // Still proceed with simulation mode
                    setMccSocket({});
                }
            };
            initMccSocket();
...
//# sourceMappingURL=%5Broot%20of%20the%20server%5D__2dc7bbc9._.js.map